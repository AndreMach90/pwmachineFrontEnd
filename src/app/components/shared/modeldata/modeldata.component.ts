...
export class ModeldataComponent implements OnInit {
  ...
  public tipoTroubleForm = new FormGroup({
    tpTrouble: new FormControl('')
  })
  ...
  filtrarTransaccionesFueraDeRango(): void {
    // Tomamos las fechas para concatenarlas más adelante
    let di: any = this.exportdateform.controls['dateini'].value;
    let df: any = this.exportdateform.controls['datefin'].value;
    let hi: any = this.exportdateform.controls['horaini'].value;
    let hf: any = this.exportdateform.controls['horafin'].value;
  ...
  limpiarListas() {
    ...
    this.listaResagadas = [];
    this.listaEquipo = [];
    this.listaEquipoGhost = [];
    this.listaCuadreEquipos = [];
  }

  obtenerEquipos(idcli: any) {
    this.limpiarListas()
    if (idcli) {
      this.clientelista.filter((cli: any) => {
        if (idcli == cli.codigoCliente) {
          this.idCli = cli.id;
        }
      });
  ...
  obtenerConsolidado(type: number) {
    ...
    console.warn('>>>>>>>>>>>>>>>>>>>>>>>CONSOLIDADO>>>>>>>>>>>>>>>>>>>>>>');
    console.warn(this.modelConsolidadoSend);
    console.warn('>>>>>>>>>>>>>>>>>>>>>>>CONSOLIDADO>>>>>>>>>>>>>>>>>>>>>>');
  ...
  async exportToExcel(): Promise<void> {
    ...
    console.log('Crear un objeto para agrupar las localidades con sus equipos y transacciones')
    console.log(localidadesMap)
    // Iterar sobre los datos para agrupar por localidad
    console.log('Iterar sobre los datos para agrupar por localidad')
  ...
  /** AÑADE LAS TRANSACCIONES A LAS CABECERAS DE DATAEXPORTAREXCEL */
  obtenerTransacTabla() {
    let x = 0;
    ...
    if (this.dataExportarExcel.length > 0) {
      ...
      Promise.all(this.dataExportarExcel.map((element: any) => {
        ...
        this._show_spinner = false;
        this.dataExportarExcel1 = this.dataExportarExcel;
        this.dataExportarExcel2 = this.dataExportarExcel;
        console.log( '------------------------------------------------------------------------' );
        console.log( 'ESTA ES LA DATA PARA EXPORTAR A EXCEL ARMADA CON LAS TRANsACCIONES' );
        console.log( this.dataExportarExcel );
        console.log( '----------------------------------1-------------------------------------' );
        console.log( this.dataExportarExcel1 );
        console.log( '----------------------------------2-------------------------------------' );
        console.log( this.dataExportarExcel2 );
        console.log( '------------------------------------------------------------------------' );
      });
    }
  }
